...
class Codon {
	...
	boolean isStop(int i) {
		if (Data.toCharArray()[i] == 'T' && ((Data.toCharArray()[i + 1] == 'A' && Data.toCharArray()[i + 2] == 'A')
				|| (Data.toCharArray()[i + 1] == 'A' && Data.toCharArray()[i + 2] == 'G')
				|| (Data.toCharArray()[i + 1] == 'G' && Data.toCharArray()[i + 2] == 'A')))
			return true;
		return false;
	}

	boolean isStart(int i) {
		if (Data.toCharArray()[i] == 'A' && Data.toCharArray()[i + 1] == 'T' && Data.toCharArray()[i + 2] == 'G')
			return true;
		return false;
	}
	...
	boolean isBigger(String a, String b) {
		if (a.toCharArray()[0] > b.toCharArray()[0])
			...
			if (a.toCharArray()[1] > b.toCharArray()[1])
				...
				if (a.toCharArray()[2] > b.toCharArray()[2])
					return true;
	...
	boolean Validate() {
		...
		for (int i = 0; i < Data.length(); i++) {
			if (Data.toCharArray()[i] != 'A' && Data.toCharArray()[i] != 'C' && Data.toCharArray()[i] != 'T'
					&& Data.toCharArray()[i] != 'G') {
	...
	int findBorder(int start, int depth) {
		for (int i = start; i < quantity; i++) {
			if (i < quantity - 1 && Codons[i].toCharArray()[depth] != Codons[i + 1].toCharArray()[depth] || depth > 0
					&& i < quantity - 1 && Codons[i].toCharArray()[depth - 1] != Codons[i + 1].toCharArray()[depth - 1])
				return i;
		}
		return quantity - 1;
	}
	...
	void sort() {
		...
		for (int depth = 1; depth < 3; depth++) {
			...
			while (left < right) {
				if (key == 'T')
					Border = findBorder(left, depth - 1);
				licznik++;
				key = genKey(licznik);
